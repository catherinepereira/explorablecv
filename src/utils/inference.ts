...
async function runOnce(modelUrl: string, input: Float32Array): Promise<InferenceResult> {
  const session = await loadModel(modelUrl)
  const tensor = new ort.Tensor('float32', input, [1, 3, CIFAR10_INPUT_SIZE, CIFAR10_INPUT_SIZE])
