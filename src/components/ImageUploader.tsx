...
export function ImageUploader() {
  ...
  return (
    <div className="flex flex-col gap-3 p-4 bg-gray-50/40 dark:bg-zinc-800/40 rounded-lg border border-gray-200 dark:border-zinc-800">
      <div className="flex flex-wrap items-center gap-2">
        <UploadButton onFile={(f) => void handleFile(f)} busy={busy} />
        <span className="text-gray-600 dark:text-zinc-400 text-sm">or try a sample:</span>
        {SAMPLE_IMAGES.map((s) => (
          <button
            key={s.url}
            onClick={() => handleSample(s.url)}
            disabled={busy}
            className="px-3 py-1 rounded-sm border border-gray-200 dark:border-zinc-800 text-gray-600 dark:text-zinc-400 hover:text-gray-900 dark:hover:text-zinc-100 hover:border-blue-600 dark:hover:border-blue-400 text-sm"
          >
            {s.label}
          </button>
        ))}
